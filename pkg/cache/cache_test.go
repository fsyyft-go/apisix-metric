...
func TestTypedCache(t *testing.T) {
	// 创建基础缓存实例
	baseCache, err := NewCache(DefaultConfig())
	if err != nil {
		t.Fatalf("创建缓存失败: %v", err)
	}
	...
	// 创建字符串类型的缓存
	strCache := NewTypedCache[string](baseCache)
	...
	personCache := NewTypedCache[Person](baseCache)
...
func BenchmarkTypedCache(b *testing.B) {
	...
	cache := NewTypedCache[User](baseCache)
